...
if(localStorage.getItem('jwt')) {
     ...
     */
    instance.default.headers.common['Authorization'] = 'Bearer' + localStorage.getItem('jwt').replace(/(^\")|(\"$)/g,'');
}
// axios拦截请求
...
export default {
    ...
    // 获取文章列表 {带分页获取}
    getArticleList(data) {
        return front_instance.post('/api/article/lists',data);
    },
    // 获取文章列表 {不带分页}
    ...
    // 获取分类列表{带权限}
    getClassify(params) {
        return instance.post('/api/classify/lists', params);
    },
    // 获取分类列表{不带权限}
    getNoAuthClass(params) {
        return front_instance.post('/api/classify/noAuth',params);
    },
    // 删除某一个分类
