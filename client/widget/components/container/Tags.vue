<template lang="html">
    <div class="tags_wrap" v-loading="loading2" element-loading-text="loading。。。。">
        <ul class="tags_list">
            <li v-for="(item, index) in items">
                <a class="tag_btn"
                   :to="`/tags/${item._id}`"
                   @click.prevent="gets(index,item.classify)"
        ...
        <transition-group name="list" tag="div">
            <article v-for="item in articleLists" v-if="show" :key="item._id">
                ...
                <footer>
                    <router-link class="tags_readMore" :to="{path:`/article/${item._id}`}">
                        阅读全文>>
                    </router-link>
                </footer>
            </article>
